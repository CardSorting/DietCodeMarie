...
| Completed task remains permanently locked after user edits message or restores checkpoint | `getTerminalCompletionEvidence` short-circuited on `durableStatus === "succeeded"` without evaluating message history reopening markers | Check `getTerminalCompletionEvidence` in `src/shared/completion/taskCompletionEvidence.ts` | Ensure `getTerminalCompletionEvidence` checks `reopensCompletedTask` and overrides durable completion when history has no completion evidence | `npx mocha --no-config --extension ts --require ts-node/register --require tsconfig-paths/register --require ./src/test/requires.cjs src/shared/completion/__tests__/taskCompletionEvidence.test.ts` passes |
